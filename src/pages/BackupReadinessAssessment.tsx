...
const BackupReadinessAssessment = () => {
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem('backupReadinessAssessment');
    if (savedProgress) {
      try {
  ...
  useEffect(() => {
    const saveProgress = async () => {
      if (Object.keys(answers).length > 0) {
        setIsSaving(true);
        try {
          localStorage.setItem('backupReadinessAssessment', JSON.stringify({
            answers,
  ...
  const handleViewResults = async () => {
    ...
    try {
      ...
      localStorage.removeItem('backupReadinessAssessment');
      
      toast.success('Assessment saved!', 'Your backup readiness assessment results have been saved successfully.');
      
      // Navigate to results page with the saved data
      navigate('/backup-readiness-results', { 
        state: { 
  ...
  const handleExportAssessment = () => {
    toast.info('Downloading template', 'CISA-aligned backup readiness assessment template is being prepared for download');
  };
