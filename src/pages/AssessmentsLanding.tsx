...
const AssessmentsLanding = () => {
  ...
  // Filter assessment tools based on active category
  const filteredAssessmentTools = React.useMemo(() => {
    if (activeFilter === 'all') {
      return assessmentTools;
    }
    return assessmentTools.filter(tool => tool.category === activeFilter);
  }, [assessmentTools, activeFilter]);

  // Get featured tools from the filtered set
  const featuredFilteredTools = React.useMemo(() => {
    return filteredAssessmentTools.filter(tool => tool.popular);
  }, [filteredAssessmentTools]);
  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredFilteredTools.length > 0 ? (
              ...
              <div className="col-span-3 text-center p-8">
                <p className="text-muted-foreground">No featured assessments found in this category.</p>
                <Button variant="link" onClick={() => setActiveFilter('all')} className="mt-2">
          ...
          <div className="space-y-4">
            {filteredAssessmentTools.length > 0 ? (
              ...
              <div className="text-center p-8 bg-muted/10 rounded-lg">
                <p className="text-muted-foreground mb-4">No assessments found in this category.</p>
