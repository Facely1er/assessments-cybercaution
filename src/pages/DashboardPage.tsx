...
const DashboardPage = () => {
  ...
  const categoryData = Object.entries(dashboardMetrics.risksByCategory)
    .map(([category, count]) => ({
      name: category,
      value: count
    }))
    ...
    .slice(0, 5); // Just show top 5 categories

  const severityData = Object.entries(dashboardMetrics.risksBySeverity)
    .map(([severity, count]) => ({
      name: severity,
      value: count
    }))
  ...
  const statusData = Object.entries(dashboardMetrics.risksByStatus)
    .map(([status, count]) => ({
  ...
  const recentActivity = [
    { id: 1, type: 'risk', action: 'New risk identified', object: 'Data Breach Due to Insecure API', time: '2 hours ago', user: 'Jane Smith' },
    { id: 2, type: 'assessment', action: 'Risk assessment completed', object: 'Unauthorized Access to Critical Systems', time: '1 day ago', user: 'John Doe' },
    { id: 3, type: 'treatment', action: 'Treatment plan updated', object: 'Regulatory Non-Compliance', time: '2 days ago', user: 'Jane Smith' },
    { id: 4, type: 'assessment', action: 'Ransomware assessment', object: 'Organization-wide assessment', time: '3 days ago', user: 'Bob Johnson' },
  ];
  ...
  const CATEGORY_COLORS = {
    ...
    [RiskCategory.SupplyChain]: '#1E40AF',
    [RiskCategory.ThirdParty]: '#3B82F6',
  };
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'assessment':
        return <FileText className="h-5 w-5 text-electric-blue" />;
      case 'treatment':
        return <CheckCircle className="h-5 w-5 text-secure-green" />;
      default:
                ...
                >
                  ...
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {categoryData.map((entry, index) => {
                      const color = CATEGORY_COLORS[entry.name as RiskCategory] || '#6B7280';
          ...
          <div className="space-y-4">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-start">
                <div className="rounded-full p-2 bg-muted mr-3 mt-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-lg font-medium">Ransomware Assessment</h3>
            </div>
            <p className="text-muted-foreground mb-4">
              Evaluate your ransomware readiness with NIST-aligned assessment.
            </p>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-muted-foreground mb-4">
              View and manage your security control implementation.
            </p>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-muted-foreground mb-4">
              Create customized risk and compliance reports.
            </p>
            <Link to="/app/reports">
              <Button className="w-full">
                Create Report
